...
- `assets/` stores bundled hashcat archives by OS/arch; `make assets` regenerates these. Generated assets live under `assets/{darwin,linux,windows}/...`.
...
- PRs should describe behavior changes, list the commands run (e.g., `make`, `go test ./...`), and note asset updates if `make assets` was rerun.
