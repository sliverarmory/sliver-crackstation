...
func wireGuardMTLSConnect(config *operatorconfig.ClientConfig) (rpcpb.SliverRPCClient, *grpc.ClientConn, error) {
...
func newWireGuardTunnel(config *operatorconfig.ClientConfig) (*wireGuardTunnel, string, error) {
	...
	dev := device.NewDevice(tun, conn.NewDefaultBind(), device.NewLogger(device.LogLevelSilent, "[crackstation/wg] "))
...
func validateWireGuardConfig(config *operatorconfig.ClientConfig) error {
	if config == nil {
